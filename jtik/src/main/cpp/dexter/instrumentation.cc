...
/**
 * zxc add for change exit hook return value type
 */
void UnBoxValueFromObject(lir::Bytecode* bytecode,
              lir::CodeIr* code_ir,
              ir::Type* type,
              dex::u4 src_reg,
              dex::u4 dst_reg) {
    ...
    check_cast->operands.push_back(code_ir->Alloc<lir::VReg>(src_reg));
    check_cast->operands.push_back(
            code_ir->Alloc<lir::Type>(boxed_type, boxed_type->orig_index));
    code_ir->instructions.InsertBefore(bytecode, check_cast);

    //eg: invoke-virtual {v0}, Ljava/lang/Integer;->intValue()
    auto ir_proto = builder.GetProto(type, nullptr);
    auto ir_method_decl = builder.GetMethodDecl(
            builder.GetAsciiString(unboxed_method_name), ir_proto, boxed_type);
    auto un_boxing_method = code_ir->Alloc<lir::Method>(ir_method_decl, ir_method_decl->orig_index);

    auto un_boxing_invoke = code_ir->Alloc<lir::Bytecode>();
    un_boxing_invoke->opcode = dex::OP_INVOKE_VIRTUAL;
    un_boxing_invoke->operands.push_back(code_ir->Alloc<lir::VReg>(src_reg));
    un_boxing_invoke->operands.push_back(un_boxing_method);
    code_ir->instructions.InsertBefore(bytecode, un_boxing_invoke);

    auto move_result = code_ir->Alloc<lir::Bytecode>();
    move_result->opcode = !is_wide ? dex::OP_MOVE_RESULT : dex::OP_MOVE_RESULT_WIDE;
...
void GenerateShiftParamsCode(lir::CodeIr* code_ir, lir::Instruction* position, dex::u4 shift) {
  const auto ir_method = code_ir->ir_method;
  SLICER_CHECK(ir_method->code->ins_count >= 0);//zxc change from >0
...
bool EntryHook::InjectArrayParamsHook(lir::CodeIr* code_ir, lir::Bytecode* bytecode) {
  ...
  if (needsExtraRegs) {
    ...
    ALOGI("transform: add %d registers in entry", regs_count - non_param_regs);
  }
  ...
  const_size_op->operands.push_back(code_ir->Alloc<lir::Const32>(
      1 + param_types.size())); // method signature + params + "this" object (zxc: - method signature)
  ...
  hook_param_types.push_back(builder.GetType("J"));
  ...
  dex::u4 method_id_value_reg = array_size_reg;//reuse
  auto const_wide_methodId = code_ir->Alloc<lir::Bytecode>();
  const_wide_methodId->opcode = dex::OP_CONST_WIDE;
  const_wide_methodId->operands.push_back(code_ir->Alloc<lir::VRegPair>(method_id_value_reg));
  const_wide_methodId->operands.push_back(code_ir->Alloc<lir::Const64>(j_method_id_));
  code_ir->instructions.InsertBefore(bytecode, const_wide_methodId);

  auto hook_method = code_ir->Alloc<lir::Method>(ir_method_decl, ir_method_decl->orig_index);
  auto args = code_ir->Alloc<lir::VRegRange>(method_id_value_reg, 3);
  auto hook_invoke = code_ir->Alloc<lir::Bytecode>();
  hook_invoke->opcode = dex::OP_INVOKE_STATIC_RANGE;
  hook_invoke->operands.push_back(args);
  hook_invoke->operands.push_back(hook_method);
  ...
  if (needsExtraRegs) {
    ...
    ALOGI("transform: shift back %d registers in entry", regs_count - non_param_regs);
  }
  return true;
}
...
bool ExitHook::Apply(lir::CodeIr* code_ir) {
  ...
  bool return_as_object = (tweak_ & Tweak::ReturnAsObject) != 0 && declared_return_type->GetCategory() == ir::Type::Category::Reference;
  ...
  // construct the hook method declaration
  std::vector<ir::Type*> param_types;
  param_types.push_back(builder.GetType("J"));
  bool isStatic = (ir_method->access_flags & dex::kAccStatic) != 0;
  if (!isStatic) {//this object
  ...
  auto setRegs = getOrgRegCount();
  auto regThisObjIndex = (setRegs != -1 ? setRegs : ir_method->code->registers) - ir_method->code->ins_count;
  ...
  for (auto instr : code_ir->instructions) {
    ...
    if (isVRegEnough) {
      ...
      if (paramBegin > ir_method->code->registers-2) {// pram not enough,then scratch
        ...
        ALOGI("transform: scratch %d registers in exit", scratch);
      }
      regIndexMethodId = paramBegin;
    }
    ...
    ALOGI("trasnsform: jmethod id in reg %d as %d", regIndexMethodId,isVRegEnough);
    ...
    code_ir->instructions.InsertBefore(bytecode, const_wide_op);

    auto args = code_ir->Alloc<lir::VRegRange>(reg, reg_count);//org return
    ...
    if (reg_list_contiguous) {
      ...
      hook_invoke->operands.push_back(v_reg_range);
    } else {
      hook_invoke->opcode = dex::OP_INVOKE_STATIC;
      hook_invoke->operands.push_back(v_reg_list);
    }
    hook_invoke->operands.push_back(hook_method);
    code_ir->instructions.InsertBefore(bytecode, hook_invoke);
...
bool MethodInstrumenter::InstrumentMethod(ir::EncodedMethod* ir_method) {
  ...
  for (const auto& transformation : transformations_) {
    transformation->setOrgRegCount(regs);
...
bool MethodInstrumenter::InstrumentMethod(const ir::MethodId& method_id) {
  ...
  return InstrumentMethod(ir_method);
    ...
    void Transformation::setOrgRegCount(int org_reg_count) {
      orgRegsCount = org_reg_count;
    }

    int Transformation::getOrgRegCount() const {
      return orgRegsCount;
    }
}  // namespace slicer
